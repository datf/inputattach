...
static int dualnot_init(int fd, unsigned long *id, unsigned long *extra)
{
    ...
    while (!readchar(fd, &data, 100) && idx < 20)
        cmd[idx++] = data;
    for (i = 0; i < idx; ++i)
        printf(" %02X", cmd[i]);
    printf("\n");    
	if (ioctl(fd, TIOCMSET, &first))
		return -1;
    return 0;
}
...
{ "--dualnot",	"-nn",	"DualNot serial touchscreen",
	B9600, CS8,
	0x40,	0x00,	0x00,	1,	dualnot_init },
...
int main(int argc, char **argv)
{
    ...
    printf("Going for initialization...\n");
